...
function fn_trans(id, target_fn, targetfn_content, targetfn_title) { // 将一个函数窗口过渡到另一个函数窗口。
    ...
    window.style.transition = `all 350ms ${easing}`;
    square.style.transition = `all 350ms ${easing}`;
    content.style.transition = `all 350ms ${easing}`;
    title.style.transition = `all 350ms ${easing}`;
